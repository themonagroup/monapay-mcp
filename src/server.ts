...
export function createServer(getClient: () => MonaPayClient = () => MonaPayClient.fromEnv()) {
  ...
  server.registerTool('monapay_quickstart', { title: 'Bat dau nhan tien: kiem tra trang thai va cac buoc tiep theo', description: 'DIEM VAO cho y dinh "toi muon nhan tien chuyen khoan tu dong". Goi tool nay TRUOC: no kiem tra tai khoan (da noi ngan hang/VA chua, da co webhook chua) va tra ve cac BUOC TIEP THEO cu the + recipe end-to-end. / Entry point for "I want to receive bank transfers": checks state and returns concrete next steps + recipe.' }, () => run(async (c) => {
    const w: any = await c.whoami();
    const d = (w && w.data) || {};
    const hasBankVA = (Number(d.bank_accounts) > 0) && (d.virtual_accounts === null || Number(d.virtual_accounts) > 0);
    const hasChannel = Number(d.webhooks) > 0;
    const ready = d.next_step === 'San sang nhan tien' || d.next_step === 'Sẵn sàng nhận tiền';
    return {
      account: { name: d.name, username: d.username, plan: d.plan },
      state: { bank_accounts: d.bank_accounts, virtual_accounts: d.virtual_accounts, webhooks: d.webhooks, ready },
      next_step: d.next_step,
      steps: [
        { n: 1, title: 'Co API key', done: true, how: 'Neu chua: monapay_generate_key' },
        { n: 2, title: 'Noi ngan hang + tao VA', done: hasBankVA, how: 'monapay_link_bank_start -> HOI nguoi dung OTP -> monapay_link_bank_verify_otp -> monapay_notification_register -> HOI OTP lan 2 -> monapay_notification_verify_otp (khong bao gio tu doan OTP)' },
        { n: 3, title: 'Tao endpoint webhook + dang ky', done: hasChannel, how: 'monapay_generate_webhook_snippet(language) de lay code, roi monapay_create_webhook(auth_type=HMAC_SHA256, secret_key=<ngau nhien >=32 ky tu>)' },
        { n: 4, title: 'Test khong ton tien', done: false, how: 'monapay_sandbox_transaction(description="DH<order>") -> monapay_webhook_logs -> monapay_verify_signature' },
        { n: 5, title: 'Nhan tien tung don', done: false, how: 'monapay_create_qr(orderId, amount, description="DH<order>") hoac monapay_create_checkout; doi soat bang monapay_list_transactions / monapay_get_transaction' },
      ],
      note: 'Tien khong di qua MONA Pay; tien vao thang tai khoan ngan hang cua ban. MONA Pay chi doc thong bao va bao lai.',
      docs: { agent: `${DOCS}/docs/ai-agent.md`, llms: `${DOCS}/llms.txt`, webhooks: `${DOCS}/docs/webhooks/bao-mat` },
    };
  }));
  server.registerTool('monapay_get_transaction', { title: 'Tra 1 giao dich theo ma don hoac ID', description: 'Tim 1 giao dich tien vao theo transaction_code (ma don, vd DH10234) hoac id trong mot tai khoan ao; dung de kiem "don X da thanh toan chua". / Get one incoming transaction by code/id within a virtual account.', inputSchema: { virtual_account_number: z.string().describe('So VA (tu monapay_list_virtual_accounts)'), id_or_code: z.string().describe('transaction_code (ma don vd DH10234) hoac transaction id') } }, ({ virtual_account_number, id_or_code }) => run((c) => c.getTransaction(virtual_account_number, id_or_code)));
  server.registerTool('monapay_get_transactions_summary', { title: 'Tong tien vao theo khoang (KHONG phai so du vi)', description: 'Tong tien vao + so giao dich + du lieu theo ngay trong khoang chon. LUU Y: MONA Pay KHONG giu tien, khong co "so du vi"; day la tong tien da vao tai khoan ngan hang cua ban. / Incoming totals over a date range (MONA Pay holds no wallet balance).', inputSchema: { start_date: z.string().optional().describe('YYYY-MM-DD'), end_date: z.string().optional().describe('YYYY-MM-DD') } }, (q) => run((c) => c.dashboardStats(q)));
  return server;
}
